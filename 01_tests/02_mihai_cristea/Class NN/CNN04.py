...
def activation(g, function):
    if function == 'sigmoid':
        ...
        activ = softmax(g)
#    elif function == 'softmaxPrime':
#        activ = softmaxPrime(g)
    elif function == 'tanh':
        activ = tanh(g)
    elif function == 'tanhPrime':
        activ = tanhPrime(g)
    elif function == 'linear':
        activ = linear(g)
    elif function == 'linearPrime':
        activ = linear(g)
    return activ
...
class CostFunctions:
    ...
    @staticmethod
    def crossentropyPrime_multy(yhat, y):
        return (yhat - y)
...
#            self.layers[i].b = np.zeros((self.layers[i].nodes,1))
    ...
    def backwardProp(self, cost, y, batch_dim, learning_rate, lmbd, cost_function_type ):         
         L = len(self.layers) - 1
         if (cost_function_type == 'cross_entropy'):
             self.layers[L].dz = CostFunctions.crossentropyPrime(self.layers[L].a, y) 
         elif (cost_function_type == 'cross_entropySoftmax'):
             self.layers[L].dz = CostFunctions.crossentropyPrime_multy(self.layers[L].a, y) 
...
#             input()
    ...
    def fit(self, X_train, y_train, batch_dim, epochs, learning_rate, lmbd, cost_function_type = 'cross_entropy' ):
        self.init_param()
        m = y_train.shape[0]
        steps = epochs // 10 
...
#        for epoch in tqdm(range(epochs)):
        for epoch in range(epochs):
            ...
            self.Elist.append(self.cost_function(y_train, yhat, cost_function_type ))
