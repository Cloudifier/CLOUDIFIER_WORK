...
class Layer:
    def __init__(self, nodes, inputs, a, activ_f):
        self.nodes = nodes
        self.inputs = inputs
        self.theta = np.random.rand(self.inputs, self.nodes)
        self.z = a.dot(self.theta)
...
#                input()
                z.append(np.dot(X_batch, self.theta[0])) 
#                a.append(X_batch)
                a.append(activ[self.activation[0]](z[0]))
#                print ('a: \n', a, '\n z: \n', z)
#             
#                input()
                ...
                for k in range(1,len(self.layers)-1):
                    ...
                    z.append(np.dot(a[k-1], self.theta[k])) 
                    a.append(activ[self.activation[k]](z[k]))
                    print('z{ww}: \n'.format(ww=k), z[k])
                    print('a{ww}: \n'.format(ww=k-1), a[k-1])
                    print('Theta{ww}: \n'.format(ww=k), self.theta[k])
                    #print('Theta lenght : \n', len(self.theta))
                    print('k : \n', k)
                    print('{ww} : \n'.format(ww=self.activation[k]), activ[self.activation[k]](0) )
                    input()
