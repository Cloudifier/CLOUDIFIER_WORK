...
#            print('activ_f: ' ,self.layers[i].activ_f)
...
#            input()
    ...
    def backwardProp(self, cost, y, batch_dim, learning_rate ):         
         L = len(self.layers) - 1
         ...
         for l in reversed(range(1,L)):
             self.layers[l].dz = np.dot(self.layers[l+1].w.T, self.layers[l+1].dz) * activation(self.layers[l].a,self.layers[l].activ_f_prime)
             self.layers[l].dw = 1/batch_dim * np.dot(self.layers[l].dz,self.layers[l-1].a.T)
             self.layers[l].db = 1/batch_dim * np.sum(self.layers[l].dz)
             #self.layers[l-1].da = np.dot(self.layers[l].w.T, self.layers[l].dz)
#             print('w', self.layers[l].w)
#             print('b', self.layers[l].b)
...
#             print('dz', self.layers[l].dz)
#             print('dw', self.layers[l].dw)
#             print('db', self.layers[l].db)
#             print('w', self.layers[l].w)
#             print('b', self.layers[l].b)
#             input()
           

    def fit(self, X_train, y_train, batch_dim, epochs, learning_rate, lmbd=1):
        ...
        for epoch in range (epochs):
            for j in range(int(m / batch_dim)):
                ...
                self.backwardProp( cost, y_batch, batch_dim, learning_rate)
                #self.upgrade_param()
            
#            print('AL: ', self.layers[-1].a) 
#            for i in range(4):
#                print('i', i, 'a', self.layers[i].a)
            yhat = self.predict(X_train)
            
            #print('yhat: ', yhat) 
#            print('acc: ', accuracy(self.layers[-1].a, y_train))   
#            input()             
            ...
            if epoch % steps == 0 :
                print(f'#{epoch} epochs. Train accuracy: {self.acc[-1]}%, E: {self.Elist[-1]}')  
...
nn.fit(X_train.T, y_train.T, 8, 25, 0.01,lmbd=1)



yhat = nn.predict(X_test.T)
print('Train accuracy: ' + str(nn.acc[-1]))
print('Test accuracy: ' + str(accuracy(yhat, y_test.T)))
