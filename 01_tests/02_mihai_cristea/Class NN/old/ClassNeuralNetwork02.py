...
class NeuralNetwork:
    ...
    def forwardProp(self,X):
        ...
        for i in range(1,len(self.layers)):
            ...
            print('activ: ', activ[self.layers[i].activ_f])
    ...
    def backwardProp(self, cost, y, batch_dim, learning_rate ):         
         L = len(self.layers) - 1
         self.layers[L].dz = self.layers[L].a - y
         self.layers[L].dw = 1/batch_dim * np.dot(self.layers[L].dz,self.layers[L-1].a.T)
         self.layers[L].db = np.sum(self.layers[L].dz)
         for l in reversed(range(1,L)):
             self.layers[l].dz = np.dot(self.layers[l+1].w.T, self.layers[l+1].dz) * activ[self.layers[l].activ_f_prime](self.layers[l].a)
             self.layers[l].dw = 1/batch_dim * np.dot(self.layers[l].dz,self.layers[l-1].a.T)
...
nn.fit(X_train.T, y_train.T, 8, 1000, 0.1,lmbd=1)



yhat = nn.forwardProp(X_test.T)
