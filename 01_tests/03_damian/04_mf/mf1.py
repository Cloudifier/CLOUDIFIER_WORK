...
UxP_norm = np.subtract(UxP, prod_means, where = mask)
...
g = tf.Graph()
with g.as_default():
    tf_u_feats = tf.Variable(initial_value = np.random.uniform(0.005, 0.1, size = (nr_users, nr_feats)), 
                             dtype = tf.float32, name = 'user_feats')
    tf_p_feats = tf.Variable(initial_value = np.random.uniform(0.005, 0.1, size = (nr_prods, nr_feats)),
                             dtype = tf.float32, name = 'prod_feats')

    tf_y = tf.constant(UxP_norm, dtype = tf.float32, name = 'ground_truth')
    ...
    tf_loss = tf.reduce_sum(tf.pow(tf.subtract(tf_y_hat_masked, tf_y_masked), 2), 
                            name = 'basic_loss')
...
ypred = u_feats.dot(p_feats.T) + prod_means
print(ypred)