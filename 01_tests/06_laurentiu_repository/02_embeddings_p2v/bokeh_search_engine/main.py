...
class DataCollector:
  def __init__(self, filename_prods, np_path):
    ...
    self.low_dim_embs = np.load(os.path.join(np_path, 'low_dim_embs\\low_dim_embs_14.npy'))
    self.y_kmeans = np.load(os.path.join(np_path, 'checkpoints\\y_kmeans_14.npy'))
    self.top_k_indexes_cosine = np.load(os.path.join(np_path, 'checkpoints\\top_100_indexes_cosine.npy'))
    self.top_k_distances_cosine = np.load(os.path.join(np_path, 'checkpoints\\top_100_distances_cosine.npy'))
    self.top_k_indexes_euclidean = None
    self.top_k_distances_euclidean = None
    self.norm_embeddings = np.load(os.path.join(np_path, 'checkpoints\\norm_embeddings_14.npy'))
...
class Map:
  def __init__(self, plot_height, plot_width, metric):
    ...
    self.p = figure(plot_height=plot_height, plot_width=plot_width, title="",
                    tools=[hover, BoxZoomTool(), WheelZoomTool(),
                           ResetTool(), PanTool()])
...
class TopKPlot:
  def __init__(self, plot_height, plot_width, metric):
    ...
    self.p = figure(plot_height=plot_height, plot_width=plot_width, title="Fereastra in care se vor afisa rezultatele cautarii",
    ...
    self.table_topk = DataTable(source=self.source_topk, columns=columns, width=600, height=650)
...
class Server:
  def __init__(self, filename_prods, np_path):
    ...
    self.show_topk_button = Button(label="Gasire top 100 produse similare", button_type="success")
  ...
  def create_layout(self):    
    ...
    self.panels.append(Panel(title='Top 100 produse similare', child=layout_tab3))
  ...
  def select_products(self, metric):
    data = self.data_collector
    if metric is 'cosine':
  ...
  def update_topk_plots(self):
    df1, prod_id, found_id = self.select_products('cosine')
    # df2, _, _ = self.select_products('euclidean')
...
np_path_cosine = os.path.join("D:\\", "Google Drive\\_hyperloop_data\\recom_compl\\_mean_flatten_comp\\LOCALemb64_epochs15_momentum_flatten_v1")
server = Server(prods_filename, np_path_cosine)
