...
def generate_full_batch(data, window):
    span = 2 * window + 1
...
def load_transactions(csv_file = 'SORTED_TRAINS.csv', pickle_file = 'CLEANED_DATA_TRANS.p'):
...
if __name__ == '__main__':
  ...
  names = df_prods['PROD_NAME'].tolist()
  id2new_id = dict(zip(ids, newids))
  new_id2prod = dict(zip(newids, names))
  
  X_train, y_train = generate_full_batch(data = data, window = 3)

  #r = RecomP2VEmbeddings(nr_products = len(id2new_id), config_file = 'tf_config.txt')
  #r.Fit(X_train = X_train, y_train = y_train, epochs = 10, batch_size = 1024)
