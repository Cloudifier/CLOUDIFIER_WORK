      ...
      if self.USE_TF:
        try:
          ...
          with g.as_default():
            ...
            tf_I = tf.constant(np_I, dtype = tf.float32)
            tf_lmbd = tf.constant(NEQ_LAMBDA, dtype=tf.float32)
            tf_xtxI = tf.add(tf.matmul(tf.transpose(tf_X),tf_X),
                             tf_I*tf_lmbd)
            tf_xtxinv = tf.matrix_inverse(tf_xtxI)
            tf_theta = tf.matmul(tf_xtxinv, tf.transpose(tf_X))
            tf_theta = tf.matmul(tf_theta, tf_y)
              
          with tf.Session(graph = g) as session:
            #np_tftheta = session.run([tf_theta])
            np_tftheta = tf_theta.eval()
          ...
          df_vector.loc[dfindex,1:] = np_tftheta.ravel()
          df_vector.iloc[dfindex,0] = str(user_id)+" TFneq"
          dfindex += 1
        except Exception as exc:
          self._logger("TF ERROR in Inverse: {}".format(exc))
          raise
